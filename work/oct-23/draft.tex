...
\begin{term-note}
Primitive.\footnote{Not fully elucidated as \code{bfo} not specified for universals nor for instances not subject to study nor for instances not touching human endeavour. % in bfo particular = instance, universal = type, reality = space + time, category is a universal, exists is xx?
Partially elucidated as \code{bfo} claim entities are either particulars or universals, and claim no entity is both a particular and a universal.} Meant to subsume all instances in reality % bfo guide = `tracks instances in reality in a way that'
and all universals in reality and nothing else. Not fully elucidated.
\end{term-note}
\begin{note}
Certain examples of---Julius Caesar, his body mass index, WWII, Verdi's \titleit{Requiem}. Certain non-examples---numbers, sets, mathematical objects, propositions ie ideal meanings. Grey cases---xx.% eg those questionably `in reality' ie questionable existence - maybe some universals, maybe holes ? - we ought to restrict examples to our own historical domain
\end{note}
\end{term}
%
%
...
\begin{term}[Work]
\label{term:work}
An \href{http://purl.obolibrary.org/obo/IAO_0000030}{\code{iao:information content entity}} ie \code{iao:ice} which is created ie output by some \code{bfo:process} where the latter \code{bfo:s-depends on} some \code{bfo:object} which is a natural person.% or relate output itself directly to human person as opposed to via process ? - and may want to allow non-natural persons though might just reduce to natural ones - 2 oct 2023 06.42
\begin{term-note}[On \code{iao:ice}]
Derived.\footnote{Not strictly defined as \code{iao:ice} is specified with an only partially elucidated \code{iao:is about} relation.% which relation includes non-entity things as relata eg configurations
} Meant to subsume xx. See \href{https://doi.org/10.3233/AO-210246}{doi:10.3233/AO-210246} for review, \href{https://hal.science/hal-03484145}{hal:03484145} for proposed extension to utterances, \href{https://ceur-ws.org/Vol-3155/short5.pdf}{ceur:v3155short5} for xx, \href{https://doi.org/10.3233/FAIA210370}{doi:10.3233/FAIA210370} for critique and proposed alternative.
\end{term-note}
\begin{term-note}[On qualifier]% or have remarks for each part of
Derived.\footnote{Not fully elucidated as xx.} Meant to subsume xx. Meant to exclude those \code{iao:ices} not relevant to us eg DNA sequences.
\end{term-note}
\begin{note}
Certain examples of---database entries input by him, xx. Certain non-examples---xx. Grey cases---xx.% grey eg iao:ices which fail the iao:is_about relation eg fake news ? stories about myths ?
\end{note}
\end{term}
%
%
...
\begin{term}[\(W\)]
\label{term:w}
The subclass of \(T\) with \sout{\code{frbr:works}} \href{http://purl.obolibrary.org/obo/IAO_0000030}{\code{iao:information content entities}} ie \code{iao:ices} \sout{and \href{http://purl.obolibrary.org/obo/IAO_0000015}{\code{ioa:information carriers}} and \href{http://purl.obolibrary.org/obo/IAO_0000178}{\code{iao:material information bearers}}}.% not of ioa:information content entities as some frbr:works might not be ioa:ice's ?
\begin{term-note}
Clean or sharp bounds for \(W\) within its container class may be useful to us. It seems it would take a bit of work to carve out such a clean shape for \(W\) though, at least with \code{frbr}.% eg ceramic midden vs shards vs ceramics themselves
\end{term-note}
\begin{term-note}
...
\end{term-type}% for pub
\end{term-type}% for man
\end{term}
%
%
%
\begin{term}[Series]
\label{term:series}
An \code{frbr:work} which is in \(W_n\). % refine this defn after filling in term:w - 1 oct 2023 21.53
 May be characterised by extent of structure (a bit vs a lot), by machine-readability (is vs is not machine-readable), by location of subject matter (subject in \(\Gamma\) vs in \(W\)). % for W_n = scholarly published works with manifestations
