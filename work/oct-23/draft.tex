...
%
%
\subsection{Version 1.0.0-alpha}
\label{ss:ver1-alpha}
This is a pre-release candidate version of the data available at \href{https://github.com/aenavidad/historical-series-re-Belize/tree/main/v-1.5}{\code{*/v-1.5}}. It consists of a human-readable (but not machine-readable) text copy of all data which will be present in version 1.0.0 of data, and was sourced from pre-git-hist as per the osf pre-registration.
%
%
%
%
...
%
% process is an occurrent which has proper temporal parts (is not just an instant) and p s-depends on some material entity
% so process = drafting of the poem resulting in this manuscript \neq drafting of the poem resulting in that manuscript
%
...
\end{term-type}% chronicle
\begin{term-type}[Dictionary]
An empirical series which xx
\end{term-type}% dictionary
\begin{term-type}[Gazetteer]
An empirical series which xx
\end{term-type}% gazetteer
\begin{term-type}[Compendium]
An empirical series which xx
\end{term-type}% compendium
\end{term-type}% empirical
...
\end{term-type}% calendar
\begin{term-type}[Catalogue]
A non-empirical series which xx
\end{term-type}% catalogue
\end{term-type}% non-empirical
\end{term}
%
%
%
