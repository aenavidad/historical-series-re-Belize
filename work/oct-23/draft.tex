...
\theoremstyle{remark}
\newtheorem*{term-elun}{Elucidation}
...
\begin{term-note}
Is primitive. Is meant to subsume all instances in reality % bfo guide = `tracks instances in reality in a way that'
and all universals in reality and nothing else. Is primitive. Is not fully elucidated.\footnote{\code{bfo} categories not specified for universals nor for instances not subject to study nor for instances not touching human endeavour.} % in bfo particular = instance, universal = type, category is a universal
...
\begin{term}[\(T\)]
\label{term:t}
The class of \href{http://purl.obolibrary.org/obo/BFO_0000001}{\code{bfo:entities}}.% but does bfo include abstract things eg frbr:works?
\begin{term-note}
...
A structured series which is machine-readable.% by which standard - eg structured data, has metadata, conforms to so-and-so standards
\end{term-type}
\end{term-type}
\end{term}
%
