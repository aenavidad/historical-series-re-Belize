...
%
exists only so long as some physical copy of it exists somewhere, like as ink-on-paper or electrons-on-silicon or chemicals-in-neurons (ie the \code{iao:ice} may \mention{migrate} from copy to copy, in contrast to entities which \code{bfo:s-depend on} something, which cannot so migrate but are rather \mention{stuck} to a single copy). If an \code{iao:ice} \code{bfo:g-depends on} some \code{bfo:ic} then there is some \code{bfo:sdc} which \code{bfo:concretises} the \code{iao:ice} and which \code{bfo:s-depends on} the \code{bfo:ic}. Eg a visual image of some physical copy of the poem (shape ie arrangement ie pattern of black strokes and serifs on whitespace as found on 
this-or-that copy ie examplar of the poem) %
%
% where if x s-depends on y then x shares no parts with y - and x cannot exist sans y
% where if x s-depends on y and y on z then x s-depends on z - ie transitive
% where no sdc is extended in space ie all sdc are immaterial - where an ic may be material or immaterial
% so we have sdc = `the shape ie form of one' \neq `the shape of \textit{one}' \neq `the shape of \textcolour{red}{one}' etc (easy cases) - but also `the shape of this one' \neq `the shape of this other one' (hard cases) - seems correct per bfo guide s 3.8 pp63-64
% and we have ic = this particular copy (electrons on silicon here) of `one' \neq that particular copy (electrons on silicon there) of `one' (easy cases) - \sout{ NO = this particular print run ie edition of `one' \neq that particular print run of `one' ? (hard cases) = NO } - factcheck 2 oct 2023 14.05
% QUOTE = BFO QUIDE P64 = think of gdc (gdc eg the chessboard pattern, the Coca Cola logo), intuitively, as complex continuant patters (complex qualities) of the sort created by authors or designer, or (in the case of DNA sequeneces) through the processes of evolution. ... gdc exist only if concretised in some counterpart sdc (sdc eg the pattern of black and white squares on this wooden chessboard here, the patters of red and white swirls on the label of this Coca Cola bottle) ... A certain pattern (of letters of the alphabet and associated punctuation and spacing) which is a work of literature is concretized in the patterns of ink marks in this and that particular copy of the work. When you create a novel, you first of all create an s-dependent pattern of inkmarks on your manuscript, a pattern that only exists when ink marks are spread around in a certain way. In addition, you create also a particular instance of the generically dependent continuant type novel. When you print further copies in book form, then you create multiple particular instances of the independent continuant type book. = P64 GUICE BFO = QUOTE
% ie for quote above - gdc = a string of symbols `one', sdc_0 = shape of black glyphs `o' and `n' and `e' in this screen here, sdc_1 = shape of black glyphs `o' and `n' and `e' in that screen there, such that sdc_i concretises gdc
% ie cont for quote above - gdc as above, sdc_0 = pattern of inkmarks spread so-and-so on your manuscript, such that sdc_0 concretises gdc
% ie cont for quote above - gdc as above, sdc_0 as above, such that sdc_0 is a particular instance of gdc
% ie cont for quote above - gdc as above, sdc_0 as above, where sdc_0 was taken to press and n copies printed [not facsimiles] such that sdc_1 is one print copy, ... , sdc_n is one print copy, then sdc_i is a particular instance of ic for 0<i
%
exists only so long as that particular copy (this-or-that copy of the poem) does, and furthermore, if said physical copy were itself xeroxed (producing another copy of the poem), we would also have a copy of the visual image of the non-xeroxed copy of the poem. %
%
